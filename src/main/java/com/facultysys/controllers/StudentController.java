...
public class StudentController {
    ...
    @GetMapping(value = "/studentby-pid/{id}")
    public ResponseEntity<StudentModelView> getStudentByState(@PathVariable int pid){
        return new ResponseEntity<>(studentService.getStudentByPid(pid),HttpStatus.OK);
    }

    @GetMapping(value = "/studentby-nationalid/{id}")
    public ResponseEntity<StudentModelView> StudentByNationalId(@PathVariable int nationalId){
